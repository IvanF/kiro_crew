...
def _extract_all_balanced_json(text: str) -> list[str]:
    ...
    Если встречается несбалансированный блок (например CSS { ... { ... } без
    закрывающей внешней скобки) — он пропускается, поиск продолжается дальше.
    ...
    while pos < len(text):
        ...
        if end is not None:
            ...
            # Несбалансированный блок (CSS, tool-output и т.д.) — пропускаем
            # и продолжаем поиск со следующего символа после {
            pos = start + 1
